...
export const registerAgenticSamplingTool = (server: McpServer) => {
  ...
  console.error("[agentic-sampling] Registering - client supports sampling.tools");
  ...
  server.registerTool(name, config, async (args, extra): Promise<CallToolResult> => {
    ...
    while (iteration < maxIterations) {
      iteration++;
      console.error(`[agentic-sampling] Iteration ${iteration}/${maxIterations}`);
      ...
      console.error(`[agentic-sampling] Got response with stopReason: ${result.stopReason}`);
